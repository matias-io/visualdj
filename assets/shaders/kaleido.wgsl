...
fn fs_main(in: VsOut) -> @location(0) vec4<f32> {
    ...
    a = abs(((a + t * 0.12) % seg + seg) % seg - seg * 0.5);
    let p = vec2<f32>(cos(a), sin(a)) * r;

    // Domain-warped flow field. Kept broad and slow: fine, fast warping everywhere at
    // once is what makes a kaleidoscope nauseating rather than hypnotic.
    let warp = 0.35 + 0.5 * mid();
    let q = vec2<f32>(fbm(p * 1.2 + vec2<f32>(t * 0.5, -t * 0.35)), fbm(p * 1.2 + vec2<f32>(-t * 0.3, t * 0.5 + 3.1)));
    let field = fbm(p * 1.8 + q * warp * 1.6 + vec2<f32>(0.0, -t * 0.5));

    // Many fine contour lines; each line takes its colour from where it sits.
    let density = 5.0 + 2.0 * treble();
    let rings = log(r + 0.05) * (2.0 + 1.0 * bass()) - t * 0.8;
    ...
    let prev_uv = (in.uv - vec2<f32>(0.5)) * (0.985 - 0.008 * bass()) + vec2<f32>(0.5);
    let prev = prev_frame(prev_uv);
    col = col + prev * (0.25 + 0.25 * trails()) * 0.8;
